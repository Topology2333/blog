...
vec operator 比较多余。vec 把一个指标对映射成一个单指标，根据原文的约定，对于 $\phi(i,j)$ 而言，前面的因子是慢指标，后面的因子是快指标（因为按列堆叠）。因此实际上对于矩阵 $A^i_j$ 而言，$\operatorname{vec} A$ 的行指标 $i$ 是快指标而列指标 $j$ 是慢指标，于是 $(\operatorname{vec} A)^{\phi (j,i)} = A^i_j$。而对于张量积的约定，很自然地有前面的指标是慢指标而后面的指标是快指标，于是很自然地有 $A^i_j\otimes B^k_l = C^{ik}_{jl} :\sim C^{\phi(i,k)}_{\phi(j,l)}$
